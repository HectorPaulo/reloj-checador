...
const IniciarModal = ({ isOpen, onRequestClose, inputValue, setInputValue, handleModalSubmit }) => {
  ...
  }, [isOpen, handleModalSubmit, onRequestClose, inputValue]);
  ...
  return (
    <Modal
      ...
      contentLabel="Nombre de la Actividad"
    ...
    >
      <h2 className="text-white text-xl font-semibold mb-4">Ingrese el nombre de la actividad</h2>
      <input
        ref={inputRef}
        type="text"
        value={inputValue}
      ...
      <button onClick={handleSubmit} className="px-6 py-2 bg-transparent border text-white cursor-pointer rounded hover:bg-white hover:text-black hover:scale-110">Iniciar</button>
    </Modal>
  );
};
